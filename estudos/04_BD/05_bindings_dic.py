...
sql_insert_user = (f'INSERT INTO {TABLE_NAME} (name, weight)'
                    'VALUES (:name, :weight)') # Definir name e weight define a chave  
# Passa a tratar os valores recebidos como dados e não como comandos
cursor.execute(sql_insert_user, {'name': 'João', 'weight': 70})
cursor.executemany(sql_insert_user, [{'name': 'Otávio', 'weight': 120}, {'name': 'Carla', 'weight': 60}, {'name': 'Wagner', 'weight': 55}] ) # Tratando como dados
